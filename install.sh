...
echo "  Standalone app (recommended): a real grouped tray icon."
echo "  systemctl --user enable --now gloaming-tray.service"
echo "  (requires PyQt6: 'pacman -S python-pyqt6' if not already installed)"
echo "  Plasmoid: add via your panel's 'Add Widgets' dialog. It will NOT dock"
echo "  inside the System Tray's grouped icons - KDE only does that for its"
echo "  own built-in tray items - so place it directly on a panel instead."
